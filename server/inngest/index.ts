...
import { Product, User, Order, DeliveryPartner } from "../models/index.js";
...
const checkLowStock = inngest.createFunction({ id: "check-low-stock", name: "Low Stock Alert", triggers: [{ event: "inventory/stock.updated" }] }, async ({ event, step }) => {
    ...
    const product: any = await step.run("fetch-product", async () => {
        const prod = await Product.findById(productId).lean();
        return prod ? { ...prod, id: prod._id.toString() } : null;
    });

    if (!product || product.stock === null || product.stock === undefined || product.stock >= LOW_STOCK_THRESHOLD) {
...
const sendMonthlyOffers = inngest.createFunction(
    ...
    async ({ step }) => {
        const { deals, users }: { deals: any[]; users: any[] } = await step.run("fetch-deals-and-users", async () => {
            ...
            return {
                deals: products.map((p: any) => ({ ...p, id: p._id.toString() })),
                users: allUsers.map((u: any) => ({ ...u, id: u._id.toString() })),
            };
        });
...
const autoAssignRider = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        const result = await step.run("assign-rider", async () => {
            ...
            // Find active riders not currently delivering
            ...
            const busyRiderIds = busyOrders.map((o) => o.deliveryPartnerId).filter(Boolean);
            ...
            order.deliveryPartnerId = availableRider._id;
